              ...
              <ul className="flex mb-2 flex-wrap">
                {
                  category.selectedOptions.map(option => (
                    <Tag
                      ...
                      title={option}
                      onRemove={() => category.onSelect(category.selectedOptions.filter(o => o != option))}
                    />
                  ))
                }
              </ul>
...
function FilterCategory({ name, options, onSelect }) {
  return (
    <div className="w-1/3 border-l border-black pl-4">
      ...
      {
        options.map(option => (
          <li role="button" className="mb-2 cursor-pointer font-semibold select-none" onClick={() => onSelect(option)}>
            {option}
          </li>
        ))
      }
